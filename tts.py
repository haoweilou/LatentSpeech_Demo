...
class StyleSpeech(nn.Module):
    ...
    def __init__(self,config,fuse_step = 0,output_dim=16):
...
class AudioLoss(nn.Module):
    ...
    def forward(self, audio,predict_audio,ae:AE,device="cuda"):
        
        mb_auido = ae.pqmf(audio)
        mb_predict_audio = ae.pqmf(predict_audio)
        b,c,t = mb_auido.shape
        mb_predict_audio = mb_predict_audio[:,:,:t]

        mb_auido = torch.reshape(mb_auido,(b*c,1,t))
        mb_predict_audio = torch.reshape(mb_predict_audio,(b*c,1,-1))
        distance = ae.spec_distance(mb_auido,mb_predict_audio)
